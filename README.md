...
### **2026-06-24: 自定义接口图片请求兼容与错误通知改进**
- **图片请求格式兼容：** 自定义接口的图片请求改为 Gemini 原生 REST 字段格式（`inlineData` / `mimeType`），修复部分兼容接口在文本请求正常但图片请求返回权限错误的问题。
- **部分成功错误通知：** 当同一条消息中部分内容已翻译成功、但图片子任务或鉴权类关键子任务失败时，机器人现在会发送受冷却时间限制的 Webhook 告警，不再只写入日志。
- **错误通知副作用修复：** 错误通知发送逻辑不再自行重启消息队列，避免通知路径改变队列处理状态；队列任务的重启仍由全局任务错误处理流程负责。
- **通知内容保护：** 过长的错误详情会在发送前截断，避免超过 Discord Embed 描述长度限制。
