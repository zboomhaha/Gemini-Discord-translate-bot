...
- **双层超时保护：** 单次 API 调用超时 10 秒（快速切换 Key/Model），此过程会复用超时后延迟到达的 200 OK，并在尝试下一个 key 前检查它是否已完成；如果之前 Key的请求已经返回 200 OK则直接使用。如果 3 个都超时，还有 5 秒的最终等待机会。单条消息整体处理超时 90 秒（防止队列阻塞），超时时自动发送错误 Webhook，若为 DM 消息则同时向用户回复"服务繁忙中，请稍后重试"。
